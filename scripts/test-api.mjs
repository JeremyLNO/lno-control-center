...
// Generated by hand or by the cron, a report is the same artefact — including its HTML
// preview. Manual generation used to skip the body, so the HTML button was permanently
// disabled on every row an admin produced from the Reports page.
for (const id of [genReport.id, genWeekly.id]) {
  r = await call(snapshots, { method: 'GET', headers: authH, query: { report: String(id), format: 'html' } });
  ok(`a manually generated report has an HTML preview (#${id})`, r.status === 200 && (r.body.html || '').includes('<'), r.status);
}
...
// ---------------------------------------------------------------------------------------
// Monthly review (api/_lib/weeklyReport.js#buildMonthlyReview). This is the shareholder-
// facing kind, so it has to roll up per fund AND per bot and still add back up to the whole.
// ---------------------------------------------------------------------------------------
await db.query("DELETE FROM trades WHERE symbol LIKE 'MO%'");
const { buildMonthlyReview } = await import('../api/_lib/weeklyReport.js');
const moFund = 'fund-monthly-test';
await db.query("INSERT INTO funds (id,name,color,bots,sort) VALUES ($1,'Monthly Test','#123456','[]'::jsonb,99)", [moFund]);
let moId = 7000;
const moTrade = (symbol, net, daysAgo, fundId = null, slip = null, rm = null) => db.query(
  `INSERT INTO trades (exchange,symbol,open_trade_id,close_trade_id,direction,qty,entry_price,exit_price,
     gross_pnl,commission,funding,net_pnl,opened_at,closed_at,duration_s,entry_hour,entry_dow,fill_count,leverage,fund_id,slippage,r_multiple)
   VALUES ('binance',$1,$2,$2,'LONG',1,100,110,$3,0,0,$3, now() - ($4||' days')::interval, now() - ($4||' days')::interval + interval '1 hour',
     3600,10,1,2,5,$5,$6,$7)`, [symbol, ++moId, net, String(daysAgo), fundId, slip, rm]
);
// This month: MOAAA +300 (in a fund, with execution metrics) and -100 (no fund), MOBBB +50.
await moTrade('MOAAA', 300, 3, moFund, -12, 2);
await moTrade('MOAAA', -100, 4, null);
await moTrade('MOBBB', 50, 5, moFund);
// Previous month: +125, so the month-over-month comparison has a real baseline.
await moTrade('MOAAA', 125, 40, moFund);

const mv = await buildMonthlyReview();
ok('monthly review reports the month net PnL', mv.portfolio.netPnl === 250 && mv.portfolio.trades === 3, mv.portfolio);
ok('monthly review compares against the previous month',
  mv.portfolio.prevNetPnl === 125 && mv.portfolio.vsPrevPct === 100, { prev: mv.portfolio.prevNetPnl, pct: mv.portfolio.vsPrevPct });
// The fund breakdown is what shareholders read: it must partition the month, not sample it.
ok('monthly review splits the result per fund', mv.funds.find(f => f.name === 'Monthly Test')?.netPnl === 350, mv.funds);
ok('a trade whose bot has no fund is kept in its own bucket rather than dropped',
  mv.funds.find(f => f.id === '__unassigned')?.netPnl === -100, mv.funds);
ok('the fund breakdown adds back up to the portfolio',
  Math.round(mv.funds.reduce((s, f) => s + f.netPnl, 0) * 100) === Math.round(mv.portfolio.netPnl * 100), mv.funds.map(f => f.netPnl));
ok('monthly review attributes PnL per bot with its own month-over-month change',
  mv.bots.find(b => b.symbol === 'MOAAA')?.netPnl === 200 && mv.bots.find(b => b.symbol === 'MOAAA')?.prevNetPnl === 125, mv.bots);
// The month has a shape: how many days traded, how many green, best and worst.
ok('monthly review describes the month day by day', mv.daysTraded === 3 && mv.greenDays === 2 && mv.bestDay?.pnl === 300 && mv.worstDay?.pnl === -100,
  { traded: mv.daysTraded, green: mv.greenDays, best: mv.bestDay, worst: mv.worstDay });
// Execution quality is only measurable on the trades whose orders were synced — the coverage
// travels with the number so nobody reads an average over a third of the month as the month.
ok('execution metrics carry their own coverage rather than averaging silently',
  mv.execution.slippage === -12 && Math.round(mv.execution.slippageCoverage) === 33 && mv.execution.avgRMultiple === 2,
  mv.execution);

const { buildMonthlyPdf: bmp } = await import('../api/_lib/report.js');
const moPdf = await bmp({ review: mv, equity: 1e6, pnl30: 250, openPnl: 0, exposure: 0, funds: [], positions: [], dateLabel: mv.monthLabel, series: [1, 2, 3] });
ok('the monthly PDF renders with the full review attached', Buffer.from(moPdf, 'base64').slice(0, 5).toString() === '%PDF-', moPdf.slice(0, 8));

sentMessages.length = 0;
r = await call(cronDaily, { method: 'POST', headers: authH, query: { force: 'monthly' } });
ok('the monthly cron archives a monthly PDF', r.body.sent.some(s => s.type === 'monthly-pdf' && s.archived && s.bytes > 0), r.body.sent.filter(s => String(s.type).startsWith('monthly')));
const moRow = (await db.query("SELECT id,status,html_body FROM reports WHERE kind='monthly' ORDER BY created_at DESC LIMIT 1")).rows[0];
// Shareholder-facing: unlike the weekly, it must NOT auto-verify — an admin signs it off.
ok('the archived monthly report waits for an admin to verify it', moRow && moRow.status === 'not_verified', moRow && moRow.status);
ok('the monthly HTML preview carries the detailed review, not just the headline',
  !!moRow.html_body && moRow.html_body.includes('MOAAA') && moRow.html_body.includes('Monthly Test'), moRow.html_body?.length);
