...
// Preset avatar gallery (public/avatars/style{1,2}/), grouped by style — mirrors
// api/_lib/constants.js's AVATAR_STYLES/avatarUrl. Static assets, no API round-trip needed.
const AVATAR_STYLES=[
...
// Duration to the second (System Status's alert history — an outage's real length, not
// rounded to the nearest minute the way fmtDur is for the coarser MTTA stat).
const fmtSeconds = (sec)=>{ sec=Math.round(sec); if(sec<60) return sec+'s'; const m=Math.floor(sec/60), s=sec%60; if(m<60) return m+'m'+(s?' '+s+'s':''); const h=Math.floor(m/60), mm=m%60; if(h<24) return h+'h'+(mm?' '+mm+'m':''); const d=Math.floor(h/24), hh=h%24; return d+'d'+(hh?' '+hh+'h':''); };
...
// Shared inline loader — every page/card in the app uses this same spinner (instead of a mix
// of plain "Loading…" text) so a loading state always reads the same way anywhere in the app.
function Loader({size='26',className='py-6'}: any){
  return <div className={`flex items-center justify-center ${className}`}><Quantum size={size} speed="1.9" color="#C9A24D"/></div>;
}
...
function Login(){
  ...
  useEffect(()=>{
    // gref's <div> only exists while mode==='otp-email' (the email-entry step) — when the
    // user moves on to the code-entry step and back, the div remounts fresh, so this effect
    // must re-run on that transition too, not just on clientId/lang.
