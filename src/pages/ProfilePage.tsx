  ...
  return <div className="max-w-2xl">
    ...
    <Card className="p-5 mb-4">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <Field label={t('login.email')} hint={t('profile.emailHint')} className="sm:col-span-2"><Input value={user.email} disabled className="bg-slate-50 text-slate-400"/></Field>
      </div>
      <div className="flex items-center gap-3 mt-4"><Btn onClick={saveInfo}>{t('profile.saveChanges')}</Btn>{saved&&<span className="text-sm text-success flex items-center gap-1 fadein"><Icon name="check" className="w-4 h-4"/>{t('profile.changesSaved')}</span>}</div>
    </Card>
    ...
    {user.authProvider==='google'
      ...
      : <Card className="p-5 mb-4">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <Field label={t('profile.currentPassword')}><Input type="password" value={pw.cur} onChange={e=>setPw({...pw,cur:e.target.value})}/></Field>
            <Field label={t('profile.newPassword')}><Input type="password" value={pw.n1} onChange={e=>setPw({...pw,n1:e.target.value})}/></Field>
            <Field label={t('profile.confirmNew')}><Input type="password" value={pw.n2} onChange={e=>setPw({...pw,n2:e.target.value})}/></Field>
          </div>
          {pw.n1&&<div className="grid grid-cols-2 sm:grid-cols-3 gap-x-3 gap-y-0.5 mt-2">
            {PW_RULES.map(([label,fn])=>{ const ok=fn(pw.n1); return <div key={label} className={`flex items-center gap-1.5 text-[11px] ${ok?'text-success':'text-slate-400'}`}><Icon name={ok?'check':'x'} className="w-3 h-3 shrink-0"/>{t(label)}</div>; })}
          </div>}
          <div className="flex items-center gap-3 mt-4"><Btn onClick={changePw}>{t('profile.updatePassword')}</Btn>
    ...
    <Card className="p-5">
      <SectionTitle>{t('profile.whatsappNotifTitle')}</SectionTitle>
      <div className="flex items-center justify-between mb-3">
        <div><div className="text-sm font-medium text-navy">{t('profile.receiveNotifications')}</div><div className="text-xs text-slate-400">{user.role==='shareholder'?t('profile.shareholderNotifHint'):t('profile.internalNotifHint')}</div></div>
        <Toggle on={notify} onChange={async x=>{setNotify(x);if(await patchSelf({notify:x})&&x&&phone)toast.success(t('profile.notifOnWelcomeSent'));}}/>
      </div>
      <div className="grid sm:grid-cols-2 gap-3">
        <Field label={t('profile.yourPhoneNumber')}><Input value={phone} onChange={e=>setPhone(e.target.value)} onBlur={async()=>{
          const firstAdd=!user.phone&&phone.trim();
          if(await patchSelf(firstAdd?{phone,notify:true}:{phone})&&firstAdd){ setNotify(true); toast.success(t('profile.notifOnWelcomeSent')); }
        }} placeholder={t('profile.phonePlaceholder')}/></Field>
      </div>
